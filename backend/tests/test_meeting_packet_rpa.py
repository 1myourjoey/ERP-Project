...
def test_analyze_root_classifies_packet_types(monkeypatch):
    root = Path(__file__).resolve().parents[1] / ".tmp_pytest" / f"meeting_packet_rpa_{uuid4().hex}"
    pex_dir = root / "1_조합원통지(25.03.06)_트리거 글로벌PEX투자조합"
    project_dir = root / "[트리거메디테크3호조합]"
    gp_dir = root / "[2025년 정기사원총회]"
    ...
    try:
        ...
        for path in [
            pex_dir / "[공문] 트리거 글로벌PEX투자조합 2기 정기조합원총회 및 반기 투자보고회.hwp",
            pex_dir / "별첨1. 의안설명서.hwp",
            pex_dir / "별첨2. 제2기 감사보고서.pdf",
            pex_dir / "별첨3. 제2기 조합 영업보고서.hwp",
            pex_dir / "별첨4. 의결권행사통보서.hwp",
            project_dir / "[공문] 트리거메디테크3호조합 제2기 정기조합원총회 개최의 건.hwp",
            project_dir / "별첨1. 의안설명서.hwp",
            project_dir / "별첨2. 제2기 감사보고서.pdf",
            project_dir / "별첨3. 제2기 조합 영업보고서.hwp",
            project_dir / "별첨4. 개정 규약(안).pdf",
            project_dir / "별첨5. 규약 신구대조표.xlsx",
            project_dir / "별첨6. 서면결의서.hwp",
            project_dir / "[트리거메디테크3호조합] 제2기 조합원총회 의사록.doc",
            gp_dir / "[공문] 트리거투자파트너스 정기사원총회 개최의 건.hwp",
            gp_dir / "별첨1. 의안설명서.hwp",
            gp_dir / "별첨2. 재무제표증명원.pdf",
            gp_dir / "별첨3. 서면결의서.hwp",
        ...
        def fake_preview(self, path: Path) -> str:
            ...
            if "PEX" in name or "투자보고회" in name:
                return "트리거 글로벌PEX투자조합 제2기 정기조합원총회 및 반기 투자보고회"
            if "트리거메디테크3호조합" in name and "공문" in name:
                return "트리거메디테크 3호조합 제2기 정기조합원총회 개최의 건\n제1호 의안 : 신규 참여인력 추가의 건\n제2호 의안 : 규약 변경의 건"
            if "신구대조표" in name:
                return "규약 변경 대조표\n변경개요: 제8조 성과보수"
            if "정기사원총회" in name:
                return "정기 사원총회 의사록\n제1호 의안 : 준법감시인 변경의 건"
        ...
        result = service.analyze_root(str(root))

        packet_map = {row.package_name: row.packet_type for row in result.packages}
        assert packet_map["1_조합원통지(25.03.06)_트리거 글로벌PEX투자조합"] == "fund_lp_regular_meeting_nongmotae"
...
def test_generation_plan_normalizes_legacy_packet_type_input(db_session, client, sample_fund):
    response = client.post(
        "/api/meeting-rpa/generation-plan",
        json={
            "fund_id": sample_fund["id"],
