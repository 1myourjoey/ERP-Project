  ...
  return (
    <div className="rounded-2xl border border-[#d8e5fb] bg-[#f5f9ff] p-4">
      <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">단계</label>
          <input
            value={value.stage || ''}
    ...
    <div className="page-container space-y-4">
      ...
      {summaryMode ? (
        <div className="card-base space-y-3">
          <h3 className="text-sm font-semibold text-[#0f1f3d]">주간 요약</h3>
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            <div className="rounded border bg-[#f5f9ff] p-3">
              <p className="text-xs text-[#64748b]">금주 신규</p>
              <p className="text-lg font-semibold text-[#0f1f3d]">{summary?.new_count ?? 0}</p>
            </div>
            <div className="rounded border bg-[#f5f9ff] p-3">
              <p className="text-xs text-[#64748b]">상태 변경</p>
              <p className="text-lg font-semibold text-[#0f1f3d]">{summary?.status_changed_count ?? 0}</p>
            </div>
            <div className="rounded border bg-[#f5f9ff] p-3">
              <p className="text-xs text-[#64748b]">코멘트 추가</p>
              <p className="text-lg font-semibold text-[#0f1f3d]">{summary?.comments_added_count ?? 0}</p>
            </div>
            <div className="rounded border bg-[#f5f9ff] p-3">
              <p className="text-xs text-[#64748b]">전체 건수</p>
              <p className="text-lg font-semibold text-[#0f1f3d]">{rows.length}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            {PIPELINE_STATUSES.map((status) => (
              <div key={status} className="rounded border p-2 text-sm">
                ...
                <span className="ml-2 font-semibold text-[#0f1f3d]">{summary?.status_counts?.[status] ?? 0}</span>
              </div>
            ))}
          </div>
          <div>
            <h4 className="mb-2 text-xs font-semibold text-[#64748b]">최근 활동</h4>
            {!summary?.recent_activities?.length ? (
              ...
              <div className="space-y-2">
                {summary.recent_activities.map((activity) => (
                  <div key={activity.review_id} className="rounded border border-[#d8e5fb] bg-white px-3 py-2 text-sm">
                    <div className="font-medium text-[#0f1f3d]">{activity.company_name}</div>
            ...
            {isLoading ? (
              ...
              <div className="grid min-w-[1200px] grid-cols-8 gap-3">
                {PIPELINE_STATUSES.map((status) => {
                  ...
                  return (
                    <div key={status} className="rounded-xl border border-[#d8e5fb] bg-[#f5f9ff] p-2">
                      <p className="mb-2 text-xs font-semibold text-[#0f1f3d]">
                          ...
                          <button
                            ...
                            className={`w-full rounded-lg border px-2 py-2 text-left text-xs transition ${
                              selectedId === row.id
                                ...
                                : 'border-[#d8e5fb] bg-white hover:border-[#bfcff0]'
                            }`}
                          >
                            <p className="font-semibold text-[#0f1f3d]">{row.company_name}</p>
                            <p className="mt-0.5 text-[#64748b]">{row.sector || '-'}</p>
                            <p className="mt-0.5 text-[#64748b]">{formatKRW(row.target_amount)}</p>
                            <p className="mt-0.5 text-[#64748b]">{row.reviewer || '-'}</p>
                          </button>
                        ))}
                        {!statusRows.length && <p className="rounded border border-dashed p-2 text-[11px] text-[#64748b]">없음</p>}
                      </div>
                    </div>
                  )
                })}
            ...
            {!selectedId ? (
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-base font-semibold text-[#0f1f3d]">{detail.company_name}</p>
                ...
                <div className="rounded border bg-[#f5f9ff] p-2 text-xs text-[#0f1f3d]">
                ...
                <div>
                  <p className="mb-1 text-xs font-semibold text-[#64748b]">코멘트</p>
                  <div className="mb-2 space-y-2">
                    {detail.comments?.length ? (
                      detail.comments.map((comment) => (
                        <div key={comment.id} className="rounded border border-[#d8e5fb] bg-white p-2 text-xs">
                          ...
                          <p className="mt-1 text-[#0f1f3d]">{comment.content}</p>
                          <p className="mt-1 text-[11px] text-[#64748b]">{toDate(comment.created_at)}</p>
                        </div>
                      ))
                    ) : (
                      <p className="text-xs text-[#64748b]">코멘트가 없습니다.</p>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <input
