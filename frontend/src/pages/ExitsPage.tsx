    ...
    <div className="page-container space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`rounded-lg px-3 py-1.5 text-sm ${activeTab === 'dashboard' ? 'primary-btn' : 'secondary-btn text-[#0f1f3d]'}`}
        >
          회수 대시보드
        </button>
        <button
          onClick={() => setActiveTab('committees')}
          className={`rounded-lg px-3 py-1.5 text-sm ${activeTab === 'committees' ? 'primary-btn' : 'secondary-btn text-[#0f1f3d]'}`}
        >
          회수 위원회
        </button>
        <button
          onClick={() => setActiveTab('trades')}
          className={`rounded-lg px-3 py-1.5 text-sm ${activeTab === 'trades' ? 'primary-btn' : 'secondary-btn text-[#0f1f3d]'}`}
        >
          회수 거래/정산
        </button>
      </div>
      ...
      {activeTab === 'dashboard' && (
        <div className="card-base space-y-3">
          <div className="flex flex-wrap items-end gap-2">
            <div>
              <label className="mb-1 block text-xs font-medium text-[#64748b]">조합 필터</label>
              <select
            ...
            <div className="rounded bg-[#f5f9ff] px-3 py-2 text-sm text-[#0f1f3d]">
      ...
      <div className="card-base">
        <h3 className="mb-2 text-sm font-semibold text-[#0f1f3d]">신규 위원회 등록</h3>
        <div className="grid grid-cols-1 gap-2 md:grid-cols-5">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">회의일</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">회수 전략</label>
      ...
      {activeTab === 'committees' && (
      <div className="card-base">
        <h3 className="mb-2 text-sm font-semibold text-[#0f1f3d]">위원회</h3>
        <div className="space-y-2">
          {committees?.map((committee) => (
            <div key={committee.id} className="rounded border border-[#d8e5fb] p-3">
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm text-[#0f1f3d]">{committee.company_name || committee.company_id} | {toDate(committee.meeting_date)} | {labelStatus(committee.status)} | {committee.exit_strategy || '-'}</p>
              ...
              {editingCommitteeId === committee.id && editCommittee && (
                <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-5">
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-[#64748b]">메모</label>
                      ...
                      <div key={item.id} className="flex items-center justify-between rounded border border-[#d8e5fb] bg-white p-2">
                        {editingCommitteeFundId === item.id && editCommitteeFund ? (
                          <div className="w-full grid grid-cols-1 gap-2 md:grid-cols-4">
                            <div>
                              <label className="mb-1 block text-xs font-medium text-[#64748b]">조합 ID</label>
                          ...
                          <>
                            <p className="text-xs text-[#64748b]">조합 {item.fund_name || item.fund_id} | 투자건 #{item.investment_id}</p>
                            <div className="flex gap-1">
      ...
      {activeTab === 'trades' && (
      <div className="card-base">
        <h3 className="mb-2 text-sm font-semibold text-[#0f1f3d]">신규 회수 거래 등록</h3>
        <div className="grid grid-cols-1 gap-2 md:grid-cols-6">
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">위원회 ID</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">회수 유형</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">거래 금액</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">매도 주식수</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">수수료</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">실현손익</label>
      ...
      {activeTab === 'trades' && (
      <div className="card-base">
        <h3 className="mb-2 text-sm font-semibold text-[#0f1f3d]">회수 거래</h3>
        <div className="space-y-1">
          {trades?.map((trade) => (
            <div key={trade.id} className="rounded border border-[#d8e5fb] p-2">
              <div className="flex items-center justify-between">
                <p className="text-sm text-[#0f1f3d]">{trade.fund_name || trade.fund_id} | {trade.company_name || trade.company_id} | {toDate(trade.trade_date)} | {labelExitType(trade.exit_type)} | 거래 금액 {formatKRW(trade.amount)} | 회수액 {formatKRW(trade.net_amount || 0)}</p>
                <div className="flex gap-1">
              ...
              {editingTradeId === trade.id && editTrade && (
                <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-6">
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">위원회 ID</label><input type="number" value={editTrade.exit_committee_id ?? ''} onChange={(e) => setEditTrade((p) => (p ? { ...p, exit_committee_id: e.target.value ? Number(e.target.value) : null } : p))} className="form-input" placeholder="위원회 ID" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">조합 ID</label><input type="number" value={editTrade.fund_id} onChange={(e) => setEditTrade((p) => (p ? { ...p, fund_id: Number(e.target.value) || 0 } : p))} className="form-input" placeholder="조합 ID" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">투자사 ID</label><input type="number" value={editTrade.company_id} onChange={(e) => setEditTrade((p) => (p ? { ...p, company_id: Number(e.target.value) || 0 } : p))} className="form-input" placeholder="투자사 ID" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">투자건 ID</label><input type="number" value={editTrade.investment_id} onChange={(e) => setEditTrade((p) => (p ? { ...p, investment_id: Number(e.target.value) || 0 } : p))} className="form-input" placeholder="투자건 ID" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">거래일</label><input type="date" value={editTrade.trade_date} onChange={(e) => setEditTrade((p) => (p ? { ...p, trade_date: e.target.value } : p))} className="form-input" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">회수 유형</label><select value={editTrade.exit_type} onChange={(e) => setEditTrade((p) => (p ? { ...p, exit_type: e.target.value } : p))} className="form-input">{EXIT_TYPE_OPTIONS.map((type) => (<option key={type} value={type}>{labelExitType(type)}</option>))}</select></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">거래 금액</label><input type="number" value={editTrade.amount} onChange={(e) => setEditTrade((p) => (p ? { ...p, amount: Number(e.target.value || 0) } : p))} className="form-input" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">매도 주식수</label><input type="number" value={editTrade.shares_sold ?? ''} onChange={(e) => setEditTrade((p) => (p ? { ...p, shares_sold: e.target.value ? Number(e.target.value) : null } : p))} className="form-input" placeholder="매도 주식수" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">수수료</label><input type="number" value={editTrade.fees} onChange={(e) => setEditTrade((p) => (p ? { ...p, fees: Number(e.target.value || 0) } : p))} className="form-input" /></div>
                  <div><label className="mb-1 block text-xs font-medium text-[#64748b]">실현손익</label><input type="number" value={editTrade.realized_gain ?? ''} onChange={(e) => setEditTrade((p) => (p ? { ...p, realized_gain: e.target.value ? Number(e.target.value) : null } : p))} className="form-input" placeholder="실현손익" /></div>
                  <div className="md:col-span-2">
                    <label className="mb-1 block text-xs font-medium text-[#64748b]">비고</label>
      ...
      >
        ...
        <label className="block">
          <span className="mb-1 block text-xs font-medium text-[#64748b]">정산 금액 *</span>
          <input
            type="number"
            min={0}
            className="form-input"
        ...
        <label className="block">
          <span className="mb-1 block text-xs font-medium text-[#64748b]">정산일 *</span>
          <input
            type="date"
            className="form-input"
        ...
        <label className="flex items-start gap-2 rounded border border-[#d8e5fb] p-3 text-sm">
          <input
            type="checkbox"
            className="mt-0.5"
