  ...
  return (
    <div className="max-w-7xl p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-slate-900">가치평가</h2>
        <button
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2">
          <button onClick={() => setFilters(EMPTY_FILTERS)} className="rounded border px-2 py-1 text-xs hover:bg-slate-100">
            필터 초기화
          </button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        <h3 className="mb-2 text-sm font-semibold text-slate-700">평가 이력</h3>
        {isLoading ? (
          <p className="p-2 text-sm text-slate-500">불러오는 중...</p>
        ) : !rows?.length ? (
          <p className="p-2 text-sm text-slate-400">가치평가 이력이 없습니다.</p>
        ) : (
                ...
                {editingId === row.id ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => setEditingId(row.id)} className="rounded bg-slate-100 px-2 py-1 text-xs hover:bg-slate-200">
                        수정
                      </button>
                      <button
                        onClick={() => {
