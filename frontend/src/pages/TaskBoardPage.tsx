...
function TaskItem({ task, onComplete, onDelete, onEdit }: {
  ...
  return (
    <div
      ...
      className="group flex items-start gap-2 p-2.5 bg-white rounded-md border border-gray-200 hover:shadow-sm transition-shadow cursor-grab active:cursor-grabbing"
    >
      <button
        onClick={() => onComplete(task.id)}
        className="mt-0.5 w-4 h-4 rounded-full border-2 border-gray-300 hover:border-green-500 hover:bg-green-50 transition-colors shrink-0"
        title="완료"
      />
...
function AddTaskForm({ quadrant, onAdd }: { quadrant: string; onAdd: (data: TaskCreate) => void }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="w-full flex items-center justify-center gap-1 py-2 text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-md transition-colors"
      >
        <Plus size={14} /> 추가
      </button>
    )
  ...
  return (
    <div className="p-2 bg-white rounded-md border border-gray-200 space-y-2">
      <input
        autoFocus
        value={title}
