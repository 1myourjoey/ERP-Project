  ...
  return (
    <div className="page-container space-y-4">
      <div className="flex items-center gap-2 text-sm text-[#64748b]">
        <Link to="/investments" className="hover:text-[#558ef8]">투자 관리</Link>
        <span>/</span>
        <span className="text-[#0f1f3d]">{companyName === '-' ? `투자 #${investmentId}` : companyName}</span>
      </div>
          ...
          {[
            ...
            <button
              ...
              className={`border-b-2 pb-2 text-sm ${activeTab === tab.key
                ? 'border-[#558ef8] font-semibold text-[#558ef8]'
                : 'border-transparent text-[#64748b] hover:text-[#0f1f3d]'}`}
            >
              {tab.label}
            </button>
          ))}
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-[#64748b]">투자를 찾을 수 없습니다.</p>
      ) : (
        <>
          {activeTab === 'overview' && (
            <div className="card-base space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-[#0f1f3d]">투자 #{selectedInvestment.id}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
              <div className="p-2 bg-[#f5f9ff] rounded">
                ...
                {selectedInvestment.fund_id ? (
                  <button
                    ...
                    className="text-xs text-[#558ef8] hover:underline"
                  >
              ...
              <div className="p-2 bg-[#f5f9ff] rounded">회사: {companyName}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">상태: {labelStatus(selectedInvestment.status || 'active')}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">투자일: {selectedInvestment.investment_date || '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">투자금액: {selectedInvestment.amount?.toLocaleString?.() ?? '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">주식 수: {selectedInvestment.shares ?? '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">주당 가격: {selectedInvestment.share_price?.toLocaleString?.() ?? '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">밸류에이션: {selectedInvestment.valuation?.toLocaleString?.() ?? '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">기존 지분율: {selectedInvestment.contribution_rate || '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">투자 라운드: {selectedInvestment.round || '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">프리머니 밸류: {selectedInvestment.valuation_pre?.toLocaleString?.() ?? '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">포스트머니 밸류: {selectedInvestment.valuation_post?.toLocaleString?.() ?? '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">지분율: {selectedInvestment.ownership_pct != null ? `${selectedInvestment.ownership_pct}%` : '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded">이사회 참여: {selectedInvestment.board_seat || '-'}</div>
              <div className="p-2 bg-[#f5f9ff] rounded md:col-span-3">투자수단: {selectedInvestment.instrument || '-'}</div>
            </div>
            </div>
          )}
          ...
          {activeTab === 'workflows' && (
            ...
            <h3 className="text-sm font-semibold text-[#0f1f3d] mb-2">연결된 워크플로우</h3>
            {!linkedWorkflows?.length ? (
              <p className="text-sm text-[#64748b]">연결된 워크플로우 인스턴스가 없습니다.</p>
            ) : (
                          ...
                          return (
                            <div
                              ...
                              className={`rounded border px-2 py-1.5 text-xs ${
                                isCurrent
                                  ? 'border-[#b2cbfb] bg-[#f5f9ff] text-[#0f1f3d]'
                                  : isDone
                                    ? 'border-emerald-200 bg-emerald-50 text-emerald-800'
                                    : 'border-[#d8e5fb] bg-white text-[#0f1f3d]'
                              }`}
                            >
                              <div className="flex items-center justify-between gap-2">
              ...
              <div className="card-base">
            <h3 className="text-sm font-semibold text-[#0f1f3d] mb-2">체크리스트</h3>
            {!linkedChecklists?.length ? (
              <p className="text-sm text-[#64748b]">연결된 체크리스트가 없습니다.</p>
            ) : (
                ...
                {linkedChecklists.map((checklist) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full text-left rounded border border-[#d8e5fb] p-2 hover:bg-[#f5f9ff]"
                    >
                      <div className="flex items-center justify-between gap-2">
          ...
          {activeTab === 'documents' && (
            ...
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold text-[#0f1f3d]">서류</h3>
            ...
            <div className="space-y-2">
              {selectedInvestment.documents?.length ? selectedInvestment.documents.map((doc) => (
                <div key={doc.id} className="border rounded p-2">
                  {editingDocId === doc.id ? (
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <p className="text-sm font-medium text-[#0f1f3d]">{doc.name}</p>
                        <p className="text-xs text-[#64748b]">{doc.doc_type || '-'} | {labelStatus(doc.status || 'pending')} | 마감 {formatDate(doc.due_date)} | {doc.note || '-'}</p>
                      </div>
              ...
              )) : <p className="text-sm text-[#64748b]">서류가 없습니다.</p>}
            </div>
            </div>
          )}
          {activeTab === 'reviews' && (
            <div className="card-base space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-[#0f1f3d]">심의 이력</h3>
                <button
              ...
              {!linkedReviewHistory.length ? (
                <p className="text-sm text-[#64748b]">연결된 심의 이력이 없습니다.</p>
              ) : (
                <div className="space-y-2">
                  {linkedReviewHistory.map((row) => (
                    <div key={row.id} className="rounded-lg border border-[#d8e5fb] bg-[#f5f9ff] p-3">
                      ...
                      <p className="mt-1 text-xs text-[#64748b]">{row.review_opinion || '-'}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          {activeTab === 'post' && (
            <>
              <div className="card-base">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold text-[#0f1f3d]">가치평가</h3>
            ...
            <div className="space-y-2">
                ...
                <div key={valuation.id} className="border rounded p-2">
                  {editingValuationId === valuation.id ? (
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-sm font-medium text-[#0f1f3d]">{formatDate(valuation.as_of_date)} | {valuation.method || '-'}</p>
                        <p className="text-xs text-[#64748b] mt-0.5">평가금액 {formatNumber(valuation.value)} | 전기 {formatNumber(valuation.prev_value)} | 증감 {formatNumber(valuation.change_amount)} ({valuation.change_pct != null ? `${valuation.change_pct}%` : '-'})</p>
                        <p className="text-xs text-[#64748b] mt-0.5">{valuation.instrument || '-'} | {valuation.evaluator || '-'} | {valuation.basis || '-'}</p>
                      </div>
                      <div className="flex gap-1">
              ...
              )) : <p className="text-sm text-[#64748b]">등록된 가치평가가 없습니다.</p>}
            </div>
              </div>

              <div className="card-base">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-[#0f1f3d]">의결권 행사 이력</h3>
              <button
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  ...
                  {!voteRecords?.length && (
                    <tr>
                      <td colSpan={6} className="px-2 py-4 text-center text-sm text-[#64748b]">
                        의결권 행사 이력이 없습니다.
                      </td>
                    </tr>
                  )}
          ...
          {activeTab === 'exit' && (
            <div className="card-base space-y-3">
              <h3 className="text-sm font-semibold text-[#0f1f3d]">회수 시뮬레이션</h3>
              <p className="text-xs text-[#64748b]">가정 배수와 성과보수율을 기준으로 순회수금과 MOIC를 계산합니다.</p>
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
                ...
                <div>
                  ...
                  <input type="number" step="0.1" value={exitMultiple} onChange={(event) => setExitMultiple(Number(event.target.value || 0))} className="form-input" />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-[#64748b]">성과보수율(%)</label>
                  <input type="number" step="0.1" value={exitFeeRate} onChange={(event) => setExitFeeRate(Number(event.target.value || 0))} className="form-input" />
                </div>
              </div>

              <div className="grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
                <div className="rounded bg-[#f5f9ff] p-2">총 회수금(가정): {formatNumber(exitSimulation.gross)}</div>
                <div className="rounded bg-[#f5f9ff] p-2">평가차익: {formatNumber(exitSimulation.gain)}</div>
                <div className="rounded bg-[#f5f9ff] p-2">성과보수: {formatNumber(exitSimulation.fee)}</div>
                <div className="rounded bg-[#f5f9ff] p-2 font-semibold text-[#1a3660]">순회수금: {formatNumber(exitSimulation.net)}</div>
...
function InvestmentForm({ funds, companies, initial, onSubmit, onCancel }: { funds: Fund[]; companies: Company[]; initial: InvestmentInput; onSubmit: (data: InvestmentInput) => void; onCancel: () => void }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 border rounded p-2 bg-[#f5f9ff]">
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">조합</label><select value={form.fund_id || ''} onChange={e => setForm(prev => ({ ...prev, fund_id: Number(e.target.value) }))} className="form-input"><option value="">조합 선택</option>{funds.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">회사</label><select value={form.company_id || ''} onChange={e => setForm(prev => ({ ...prev, company_id: Number(e.target.value) }))} className="form-input"><option value="">회사 선택</option>{companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">투자일</label><input type="date" value={form.investment_date || ''} onChange={e => setForm(prev => ({ ...prev, investment_date: e.target.value }))} className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">투자금액</label><input type="number" value={form.amount ?? ''} onChange={e => setForm(prev => ({ ...prev, amount: e.target.value ? Number(e.target.value) : null }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">주식 수</label><input type="number" value={form.shares ?? ''} onChange={e => setForm(prev => ({ ...prev, shares: e.target.value ? Number(e.target.value) : null }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">주당 가격</label><input type="number" value={form.share_price ?? ''} onChange={e => setForm(prev => ({ ...prev, share_price: e.target.value ? Number(e.target.value) : null }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">밸류에이션</label><input type="number" value={form.valuation ?? ''} onChange={e => setForm(prev => ({ ...prev, valuation: e.target.value ? Number(e.target.value) : null }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">투자 라운드</label><input value={form.round || ''} onChange={e => setForm(prev => ({ ...prev, round: e.target.value }))} placeholder="예: Series A" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">프리머니 밸류</label><input type="number" value={form.valuation_pre ?? ''} onChange={e => setForm(prev => ({ ...prev, valuation_pre: e.target.value ? Number(e.target.value) : null }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">포스트머니 밸류</label><input type="number" value={form.valuation_post ?? ''} onChange={e => setForm(prev => ({ ...prev, valuation_post: e.target.value ? Number(e.target.value) : null }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">지분율(%)</label><input type="number" step="0.01" value={form.ownership_pct ?? ''} onChange={e => setForm(prev => ({ ...prev, ownership_pct: e.target.value ? Number(e.target.value) : null }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">이사회 참여</label><input value={form.board_seat || ''} onChange={e => setForm(prev => ({ ...prev, board_seat: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">기존 지분율</label><input value={form.contribution_rate || ''} onChange={e => setForm(prev => ({ ...prev, contribution_rate: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">투자수단</label><input value={form.instrument || ''} onChange={e => setForm(prev => ({ ...prev, instrument: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">상태</label><input value={form.status || ''} onChange={e => setForm(prev => ({ ...prev, status: e.target.value }))} placeholder="예: active" className="form-input" /></div>
...
function DocumentForm({ initial, onSubmit, onCancel }: { initial: InvestmentDocumentInput; onSubmit: (data: InvestmentDocumentInput) => void; onCancel: () => void }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-5 gap-2 border rounded p-2 bg-[#f5f9ff] mb-2">
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">서류명</label><input value={form.name} onChange={e => setForm(prev => ({ ...prev, name: e.target.value }))} placeholder="예: 주주명부" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">유형</label><input value={form.doc_type || ''} onChange={e => setForm(prev => ({ ...prev, doc_type: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">상태</label><input value={form.status || ''} onChange={e => setForm(prev => ({ ...prev, status: e.target.value }))} placeholder="예: pending" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">마감일</label><input type="date" value={form.due_date || ''} onChange={e => setForm(prev => ({ ...prev, due_date: e.target.value || null }))} className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">비고</label><input value={form.note || ''} onChange={e => setForm(prev => ({ ...prev, note: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
...
function VoteRecordForm({
  ...
  return (
    <div className="mb-2 grid grid-cols-1 gap-2 rounded border bg-[#f5f9ff] p-2 md:grid-cols-5">
      <div>
        <label className="mb-1 block text-xs font-medium text-[#64748b]">의결 유형</label>
        <select
...
function ValuationForm({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-2 border rounded p-2 bg-[#f5f9ff] mb-2">
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">기준일</label><input type="date" value={form.as_of_date || ''} onChange={e => setForm(prev => ({ ...prev, as_of_date: e.target.value }))} className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">평가금액</label><input type="number" value={form.value ?? ''} onChange={e => setForm(prev => ({ ...prev, value: Number(e.target.value || 0) }))} placeholder="숫자 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">전기 평가금액</label><input type="number" value={form.prev_value ?? ''} onChange={e => setForm(prev => ({ ...prev, prev_value: e.target.value ? Number(e.target.value) : null }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">평가 방법</label><input value={form.method || ''} onChange={e => setForm(prev => ({ ...prev, method: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">대상 증권</label><input value={form.instrument || ''} onChange={e => setForm(prev => ({ ...prev, instrument: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">평가기관</label><input value={form.evaluator || ''} onChange={e => setForm(prev => ({ ...prev, evaluator: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">증감액</label><input type="number" value={form.change_amount ?? ''} onChange={e => setForm(prev => ({ ...prev, change_amount: e.target.value ? Number(e.target.value) : null }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">증감률(%)</label><input type="number" step="0.01" value={form.change_pct ?? ''} onChange={e => setForm(prev => ({ ...prev, change_pct: e.target.value ? Number(e.target.value) : null }))} placeholder="선택 입력" className="form-input" /></div>
      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">산정 근거</label><input value={form.basis || ''} onChange={e => setForm(prev => ({ ...prev, basis: e.target.value }))} placeholder="선택 입력" className="form-input" /></div>
      <div className="md:col-span-3 flex gap-2">
        <button
