    ...
    <div className="page-container space-y-4">
      ...
      <div className="card-base">
        <div className="mb-3 flex gap-2 text-xs">
          <button onClick={() => setTab('active')} className={`rounded px-3 py-1.5 ${tab === 'active' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}>활성 사용자 ({activeUsers.length})</button>
          <button onClick={() => setTab('pending')} className={`rounded px-3 py-1.5 ${tab === 'pending' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}>승인 대기 ({pendingUsers.length})</button>
          <button onClick={() => setTab('invites')} className={`rounded px-3 py-1.5 ${tab === 'invites' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}>초대 관리 ({invitations.length})</button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-3xl rounded-xl border border-white/30 bg-white p-4 shadow-xl">
            ...
            {(form.role === 'viewer' || form.role === 'manager') && (
              <div className="mt-3 max-h-40 overflow-auto rounded border border-gray-200 p-2 text-xs">
                {ROUTE_KEYS.map((key) => (
