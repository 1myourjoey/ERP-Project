...
function CompanyReviewCard({
  ...
  return (
    <div className="rounded-lg border border-[#d8e5fb] p-3">
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">분기 매출</label>
          <input
            type="number"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">분기 순이익</label>
          <input
            type="number"
            className="form-input"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">자본총계</label>
          <input
            type="number"
            className="form-input"
        ...
        <div className="rounded bg-[#f5f9ff] px-2 py-2 text-xs text-[#64748b]">
          추정 투자원금
          <p className="mt-1 font-semibold text-[#0f1f3d]">{formatKRW(row.paid_in_capital)}</p>
        </div>
      </div>

      <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-4">
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">건전성 등급</label>
          <select
            className="form-input"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">손상 유형</label>
          <select
            className="form-input"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">손상금액</label>
          <input
            type="number"
            className="form-input"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">투자 의견</label>
          <select
            className="form-input"
      ...
      <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">주요 이슈</label>
          <textarea
            rows={2}
            className="form-input"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-[#64748b]">후속 조치</label>
          <textarea
            rows={2}
            className="form-input"
  ...
  return (
    <div className="page-container space-y-4">
      ...
      <div className="card-base">
        <h3 className="mb-2 text-sm font-semibold text-[#0f1f3d]">내부보고회 생성</h3>
        <div className="grid grid-cols-1 gap-2 md:grid-cols-5">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">분기</label>
            <select
              className="form-input"
              value={quarter}
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[420px_minmax(0,1fr)]">
        <div className="card-base">
          <h3 className="mb-2 text-sm font-semibold text-[#0f1f3d]">목록</h3>
          {listLoading ? (
            <PageLoading />
          ) : !sortedRows.length ? (
            ...
            <div className="space-y-2">
              {sortedRows.map((row) => (
                <button
                  key={row.id}
                  type="button"
                  className={`w-full rounded border p-2 text-left ${selectedId === row.id ? 'border-[#b2cbfb] bg-[#f5f9ff]' : 'border-[#d8e5fb] bg-white'}`}
                  onClick={() => navigate(`/internal-reviews/${row.id}`)}
                >
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-sm font-medium text-[#0f1f3d]">{row.fund_name}</p>
                    <span className={badgeClass(row.status)}>{statusLabel(row.status)}</span>
                  </div>
                  <p className="mt-1 text-xs text-[#64748b]">{row.year}년 {row.quarter}분기 · 기준일 {row.reference_date || '-'}</p>
                </button>
              ))}
            </div>
          )}
          ...
          {!selectedId ? (
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <h3 className="text-base font-semibold text-[#0f1f3d]">
