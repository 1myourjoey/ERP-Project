...
export default function ReportsPage() {
  ...
  function renderFindingGroup(title: string, findings: PreReportCheckFinding[]) {
    return (
      <div className="space-y-1 rounded-xl border border-[#d8e5fb] bg-white p-3">
        <p className="text-xs font-semibold text-[#0f1f3d]">{title}</p>
        {findings.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {findings.map((finding, idx) => {
              ...
              return (
                <div key={`${title}-${idx}`} className="rounded-lg border border-[#e6eefc] bg-[#f5f9ff] px-2 py-1.5">
                  <div className="flex flex-wrap items-center gap-1">
                    <span className={severityBadgeClass(finding.severity)}>
                      {finding.severity.toUpperCase()}
                    </span>
                    <span className="text-xs font-medium text-[#0f1f3d]">{finding.title}</span>
                  </div>
                  <p className="mt-0.5 text-xs text-[#0f1f3d]">{finding.detail}</p>
                  {refText && <p className="mt-0.5 text-[11px] text-[#64748b]">근거: {refText}</p>}
                </div>
              )
            })}
          </div>
    ...
    <div className="page-container space-y-4">
      ...
      <div className="card-base">
        <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
          <div>
            <label className="mb-1 block text-xs font-medium text-[#64748b]">보고 대상</label>
            <select value={filters.report_target} onChange={(e) => setFilters((prev) => ({ ...prev, report_target: e.target.value }))} className="w-full rounded-xl border border-[#d8e5fb] px-2 py-1 text-sm">
          ...
          <div>
            ...
            <select value={filters.fund_id || ''} onChange={(e) => setFilters((prev) => ({ ...prev, fund_id: Number(e.target.value) || null }))} className="w-full rounded-xl border border-[#d8e5fb] px-2 py-1 text-sm">
          ...
          <div>
            ...
            <select value={filters.status} onChange={(e) => setFilters((prev) => ({ ...prev, status: e.target.value }))} className="w-full rounded-xl border border-[#d8e5fb] px-2 py-1 text-sm">
      ...
      {showCreate && (
        <div className="rounded-2xl border border-[#d8e5fb] bg-[#f5f9ff] p-4 space-y-2">
          <h3 className="text-sm font-semibold text-[#0f1f3d]">신규 보고 기록</h3>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-5">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-[#64748b]">기간</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-[#64748b]">마감일</label>
      ...
      <div className="space-y-3">
        {!!matrixRows.length && (
          <div className="card-base overflow-auto">
            <h3 className="mb-2 text-sm font-semibold text-[#0f1f3d]">조합별 보고 현황 매트릭스</h3>
              ...
              <tbody>
                {matrixRows.map(({ fund, byTarget }) => (
                  <tr key={fund.id} className="border-t">
                    <td className="px-2 py-2 font-medium text-[#0f1f3d]">{fund.name}</td>
                    {REPORT_TARGET_OPTIONS.map((target) => {
              ...
              <div key={row.id} className={`card-base ${highlightId === row.id ? 'ring-2 ring-blue-300' : ''}`}>
                {isEditing && editForm ? (
                  <div className="space-y-2">
                    <div className="grid grid-cols-1 gap-2 md:grid-cols-5">
                      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">보고 대상</label><select value={editForm.report_target} onChange={(e) => setEditForm((prev) => prev ? { ...prev, report_target: e.target.value } : prev)} className="form-input">{REPORT_TARGET_OPTIONS.map((target) => <option key={target} value={target}>{target}</option>)}</select></div>
                      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">조합</label><select value={editForm.fund_id || ''} onChange={(e) => setEditForm((prev) => prev ? { ...prev, fund_id: Number(e.target.value) || null } : prev)} className="form-input"><option value="">조합 미지정</option>{funds?.map((fund) => <option key={fund.id} value={fund.id}>{fund.name}</option>)}</select></div>
                      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">기간</label><input value={editForm.period} onChange={(e) => setEditForm((prev) => prev ? { ...prev, period: e.target.value } : prev)} className="form-input" /></div>
                      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">마감일</label><input type="date" value={editForm.due_date || ''} onChange={(e) => setEditForm((prev) => prev ? { ...prev, due_date: e.target.value || null } : prev)} className="form-input" /></div>
                      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">상태</label><select value={editForm.status || '예정'} onChange={(e) => setEditForm((prev) => prev ? { ...prev, status: e.target.value } : prev)} className="form-input">{STATUS_OPTIONS.map((status) => <option key={status} value={status}>{status}</option>)}</select></div>
                    </div>
                    <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">제출일</label><input type="date" value={editForm.submitted_date || ''} onChange={(e) => setEditForm((prev) => prev ? { ...prev, submitted_date: e.target.value || null } : prev)} className="form-input" /></div>
                      <div><label className="mb-1 block text-xs font-medium text-[#64748b]">메모</label><textarea value={editForm.memo || ''} onChange={(e) => setEditForm((prev) => prev ? { ...prev, memo: e.target.value } : prev)} rows={3} className="form-input" /></div>
                    </div>
                    <div className="flex gap-2">
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-[#0f1f3d]">{row.report_target} · {row.period}</p>
                      <div className="flex items-center gap-1">
                      {badge && <span className={badge.className}>{badge.text}</span>}
                        <span className={statusBadgeClass(row.status)}>{labelStatus(row.status)}</span>
                      </div>
                    </div>
                    <p className="text-xs text-[#64748b]">조합: {row.fund_name || '미지정'} | 마감일: {formatDate(row.due_date)} | 제출일: {formatDate(row.submitted_date)}</p>
                    <div>
                      <label className="mb-1 block text-xs font-medium text-[#64748b]">메모</label>
                      <textarea value={row.memo || ''} readOnly rows={3} className="w-full rounded border border-[#d8e5fb] bg-[#f5f9ff] px-2 py-1 text-sm text-[#0f1f3d]" />
                    </div>
                    <div className="flex flex-wrap gap-1">
                      ...
                      <button
                        ...
                        className={`secondary-btn ${openPreCheckId === row.id ? 'border-[#c5d8fb] bg-[#f5f9ff] text-[#1a3660]' : ''}`}
                      >
                        사전 검증
                      </button>
                    </div>
                    ...
                    {openPreCheckId === row.id && (
                      <div className="mt-2 space-y-2 rounded-xl border border-[#d8e5fb] bg-[#f5f9ff]/40 p-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div>
                            <p className="text-sm font-semibold text-[#0f1f3d]">보고서 사전 검증</p>
                            <p className="text-xs text-[#64748b]">법적 오류, 교차 검증, 가이드라인, 계약 일치성을 점검합니다.</p>
                          </div>
                        ...
                        {isPreChecksLoading ? (
                          <p className="text-xs text-[#64748b]">검증 이력을 불러오는 중입니다...</p>
                        ) : latestPreCheck ? (
                          <div className="space-y-2">
                            <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                              <div className="rounded-lg border border-[#d8e5fb] bg-white p-2">
                                <p className="text-[11px] text-[#64748b]">오류</p>
                                <p className="text-base font-semibold text-red-600">{latestPreCheck.total_errors}</p>
                              </div>
                              <div className="rounded-lg border border-[#d8e5fb] bg-white p-2">
                                <p className="text-[11px] text-[#64748b]">경고</p>
                                <p className="text-base font-semibold text-amber-600">{latestPreCheck.total_warnings}</p>
                              </div>
                              <div className="rounded-lg border border-[#d8e5fb] bg-white p-2">
                                <p className="text-[11px] text-[#64748b]">정보</p>
                                <p className="text-base font-semibold text-[#558ef8]">{latestPreCheck.total_info}</p>
                              </div>
                              <div className="rounded-lg border border-[#d8e5fb] bg-white p-2">
                                <p className="text-[11px] text-[#64748b]">시정 Task</p>
                                <p className="text-base font-semibold text-[#0f1f3d]">{latestPreCheck.tasks_created}</p>
                              </div>
                            </div>
                            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                          ...
                          <p className="text-xs text-[#64748b]">검증 이력이 없습니다. 검증 실행 버튼으로 첫 점검을 수행하세요.</p>
                        )}
                      </div>
                    )}
                  </div>
