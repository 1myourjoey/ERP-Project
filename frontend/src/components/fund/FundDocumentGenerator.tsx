...
function statusChip(status: string): { label: string; className: string } {
  ...
  if (status === 'processing') return { label: '처리 중', className: 'bg-[#e6efff] text-[#1a3660]' }
    ...
    <div className="space-y-4">
      <div className="card-base space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h3 className="text-base font-semibold text-[#0f1f3d]">📄 조합 서류 자동 생성</h3>
            <p className="mt-1 text-xs text-[#64748b]">템플릿: {totalTemplates}개 | 마커: {markerDefs.length}개</p>
          </div>
        ...
        <div className="rounded-lg border border-[#d8e5fb] bg-[#f5f9ff] px-3 py-3">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-sm font-medium text-[#0f1f3d]">생성 단계 선택</p>
            <div className="flex gap-2 text-xs">
              <button
                onClick={() => setSelectedStages(STAGE_OPTIONS.map((item) => item.value))}
                className="rounded border border-[#bfcff0] bg-white px-2 py-1 text-[#64748b] hover:bg-[#f5f9ff]"
              >
                전체 선택
              </button>
              <button
        ...
        <div className="space-y-3">
          {groupedSections.map(([section, markers]) => {
            ...
            return (
              <div key={section} className="rounded-lg border border-[#d8e5fb]">
                <button
                  onClick={() => onToggleSection(section)}
                  className="flex w-full items-center justify-between rounded-t-lg bg-[#f5f9ff] px-3 py-2 text-left"
                >
                  <span className="text-sm font-semibold text-[#0f1f3d]">{section}</span>
                  <span className="text-xs text-[#64748b]">{opened ? '접기' : '열기'}</span>
                </button>
                {opened && (
                    ...
                    {markers.map((marker) => {
                      ...
                      return (
                        <div key={marker.key} className="grid grid-cols-1 gap-1 md:grid-cols-12 md:items-center">
                          <label className="text-xs font-medium text-[#64748b] md:col-span-4">
        ...
        <div className="rounded-lg border border-[#c5d8fb] bg-[#f5f9ff] p-3">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-sm font-medium text-[#0f1f3d]">
              선택 단계 {selectedStages.length}개 / 총 {STAGE_OPTIONS.length}개
            </p>
            <p className="text-xs text-[#1a3660]">필수 미입력 {requiredMissingKeys.length}개</p>
          </div>
      ...
      <div className="card-base">
        <div className="mb-3 flex items-center justify-between">
          <h4 className="text-sm font-semibold text-[#0f1f3d]">생성 이력</h4>
          <button
            onClick={() => historyQuery.refetch()}
            className="rounded border border-[#bfcff0] bg-white px-3 py-1 text-xs text-[#64748b] hover:bg-[#f5f9ff]"
          >
            새로고침
          </button>
        </div>
          ...
          <table className="min-w-[720px] w-full text-sm">
            ...
            <tbody className="divide-y">
              {historyRows.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-3 py-8 text-center text-sm text-[#64748b]">
                    생성 이력이 없습니다.
                  </td>
                </tr>
              ) : (
