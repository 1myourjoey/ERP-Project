...
export default function LPContributionPanel({
  ...
  return (
    <div className="space-y-3 border-t border-[#d8e5fb] bg-[#f5f9ff]/30 p-3">
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm font-semibold text-[#0f1f3d]">납입 이력 · {lpName}</p>
      ...
      <div className="rounded border border-[#c5d8fb] bg-white px-3 py-2 text-xs text-[#1a3660]">
        {guideMessage}
      </div>

      {summaryQuery.isLoading ? (
        <p className="py-3 text-sm text-[#64748b]">납입 이력을 불러오는 중입니다...</p>
      ) : (
        <div className="overflow-auto rounded border border-[#d8e5fb] bg-white">
          <table className="min-w-[860px] w-full text-xs">
            ...
            <tbody className="divide-y">
              {rowsWithCumulative.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-2 py-6 text-center text-sm text-[#64748b]">등록된 납입 이력이 없습니다.</td>
                </tr>
              ) : (
                rowsWithCumulative.flatMap((row, index) => {
                  ...
                  const lineRows = [
                    <tr key={`contribution-${row.id}`}>
                      ...
                      <td className="px-2 py-2 text-[#64748b]">{row.memo || '-'}</td>
                  ...
                  if (editingId === row.id) {
                    lineRows.push(
                      <tr key={`contribution-edit-${row.id}`} className="bg-[#f5f9ff]">
                        <td colSpan={8} className="px-2 py-2">
                          <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
                            <div>
                              <label className="mb-1 block text-[11px] text-[#64748b]">납입기일</label>
                              <input
                                type="date"
                            ...
                            <div>
                              <label className="mb-1 block text-[11px] text-[#64748b]">납입금액</label>
                              <KrwAmountInput
                            ...
                            <div>
                              <label className="mb-1 block text-[11px] text-[#64748b]">실제입금일</label>
                              <input
                                type="date"
                            ...
                            <div>
                              <label className="mb-1 block text-[11px] text-[#64748b]">비고</label>
                              <input
      ...
      {showAddForm && (
        <div className="rounded border border-[#d8e5fb] bg-white p-3">
          <p className="mb-2 text-sm font-medium text-[#0f1f3d]">납입 이력 추가</p>
          <div className="grid grid-cols-1 gap-2 md:grid-cols-4">
            <div>
              <label className="mb-1 block text-[11px] text-[#64748b]">납입기일</label>
              <input
