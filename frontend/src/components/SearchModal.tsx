...
export default function SearchModal({
  ...
  return (
    <div className="modal-overlay fixed inset-0 z-[110] bg-black/40 p-4 md:p-10" onClick={handleClose}>
      ...
      >
        <div className="border-b border-[#d8e5fb] px-3 py-2">
          <label className="mb-1 block text-[10px] font-medium text-[#64748b]">통합 검색</label>
        ...
        <div className="max-h-[60vh] overflow-auto p-2">
          {loading && <p className="p-2 text-sm text-[#64748b]">검색 중...</p>}
          {!loading && query.trim() && results.length === 0 && (
            <p className="rounded-lg px-3 py-8 text-center text-sm text-[#64748b]">
              '{query.trim()}'에 대한 검색 결과가 없습니다.
            </p>
          )}
          {!query.trim() && (
            <p className="p-2 text-sm text-[#64748b]">Ctrl+Space 또는 Cmd+Space로 언제든지 검색할 수 있습니다.</p>
          )}
          ...
          {Object.entries(grouped).map(([type, items]) => {
            ...
            return (
              <div key={type} className="mb-3">
                <p className="flex items-center gap-1.5 px-2 pb-1 text-xs font-medium text-[#64748b]">
                  <HeaderIcon size={12} />
                  {meta?.label || type}
                </p>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm text-[#0f1f3d]">{item.title}</p>
                        </div>
                        {item.subtitle && <p className="mt-0.5 pl-6 text-xs text-[#64748b]">{item.subtitle}</p>}
                      </button>
                    )
                  })}
                </div>
