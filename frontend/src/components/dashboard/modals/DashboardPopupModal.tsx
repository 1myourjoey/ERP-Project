    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-[#64748b] hover:text-[#64748b]">
            ×
          </button>
        </div>
            ...
            groupedToday.map(([category, tasks]) => (
              <div key={category} className="mb-3">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-[#64748b]">{tasks.length}건</span>
                </div>
                <div className="space-y-1">
                  {tasks.map((task) => (
                    ...
                    >
                      <p className="text-sm font-medium text-[#0f1f3d]">{task.title}</p>
                      {task.deadline && <p className="mt-0.5 text-xs text-[#64748b]">{formatShortDate(task.deadline)}</p>}
                    </button>
                  ))}
                </div>
              </div>
          ...
          {popupSection === 'this_week' &&
            groupedThisWeekByDeadline.map(([dateKey, tasks]) => (
              <div key={dateKey} className="mb-3">
                <p className="mb-1 text-xs font-semibold text-[#64748b]">
          ...
          {popupSection === 'workflows' &&
            groupedWorkflows.map(([fundName, workflows]) => (
              <div key={fundName} className="mb-3">
                <p className="mb-1 text-xs font-semibold text-[#64748b]">{fundName}</p>
                <div className="space-y-1">
                  {workflows.map((workflow) => (
                    <button
                      ...
                      className="w-full rounded-lg border border-[#d8e5fb] p-2 text-left hover:bg-[#f5f9ff]"
                    >
                      <p className="text-sm font-medium text-[#0f1f3d]">{workflow.name}</p>
                      <p className="text-xs text-[#64748b]">{workflow.progress}</p>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {popupSection === 'completed' &&
            groupedCompleted.map(([category, tasks]) => (
              <div key={category} className="mb-3">
                ...
                <div className="space-y-1">
                  {tasks.map((task) => (
                    <div key={task.id} className="rounded-lg border border-[#d8e5fb] p-2">
                      <div className="flex items-center justify-between">
                        <button
                          onClick={() => onOpenTask(task, true)}
                          className="truncate text-left text-sm text-[#64748b] line-through hover:text-[#558ef8]"
                        >
                          {task.title}
                        </button>
                        <button
