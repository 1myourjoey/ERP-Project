    ...
    <div className="space-y-3">
      <div className="card-base">
        <div className="mb-2 flex items-center justify-between gap-2">
          <h3 className="text-sm font-semibold text-[#0f1f3d]">시정조치 추적</h3>
          <span className="text-xs text-[#64748b]">{isFetching ? '새로고침 중...' : '업무 연동 점검'}</span>
        </div>

        <div className="grid grid-cols-2 gap-2 lg:grid-cols-5">
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
            <p className="text-xs text-[#64748b]">전체</p>
            <p className="text-lg font-semibold text-[#0f1f3d]">{stats.total_tasks}</p>
          </div>
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
            <p className="text-xs text-[#64748b]">완료</p>
            <p className="text-lg font-semibold text-green-700">{stats.completed}</p>
          </div>
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
            <p className="text-xs text-[#64748b]">미완료</p>
            <p className="text-lg font-semibold text-amber-700">{stats.pending}</p>
          </div>
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
            <p className="text-xs text-[#64748b]">완료율</p>
            <p className="text-lg font-semibold text-[#0f1f3d]">{stats.completion_rate}%</p>
          </div>
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
            <p className="text-xs text-[#64748b]">평균 소요일</p>
            <p className="text-lg font-semibold text-[#0f1f3d]">{stats.avg_resolution_days}일</p>
          </div>
        </div>
      </div>

      <div className="card-base overflow-auto">
        <h4 className="mb-2 text-sm font-semibold text-[#0f1f3d]">기한 초과 업무</h4>
              ...
              {stats.overdue_tasks.map((row) => (
                <tr key={`${row.task_id}-${row.rule_id ?? 'no-rule'}`}>
                  <td className="px-3 py-2">
                    <div className="font-mono text-xs text-[#64748b]">#{row.task_id}</div>
                    <div>{row.task_title || '-'}</div>
                  </td>
                  <td className="px-3 py-2 text-xs text-[#0f1f3d]">{row.rule_name || '-'}</td>
