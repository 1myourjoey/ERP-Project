    ...
    <div className="space-y-3">
      <div className="card-base">
        <div className="mb-2 flex items-center justify-between gap-2">
          <h3 className="text-sm font-semibold text-gray-800">위반 패턴 분석</h3>
          ...
          <div className="space-y-2">
            {recurring.map((item) => (
              <div key={`${item.rule_id}-${item.rule_code}`} className="rounded-xl border border-gray-200 bg-white/70 p-3">
                ...
                <p className="mt-1 text-xs text-gray-600">발생 월: {monthLabel(item.months_violated)}</p>
                <p className="mt-1 text-xs text-gray-600">패턴: {item.pattern}</p>
                <p className="mt-2 text-xs text-gray-700">권고: {item.recommendation}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        <div className="card-base">
          ...
          {!improving.length ? (
            <p className="text-xs text-gray-500">개선 추이가 감지되지 않았습니다.</p>
          ) : (
            <div className="space-y-2">
              {improving.map((item) => (
        ...
        <div className="card-base">
          ...
          {!worsening.length ? (
            <p className="text-xs text-gray-500">악화 추이가 감지되지 않았습니다.</p>
          ) : (
            <div className="space-y-2">
              {worsening.map((item) => (
