  ...
  return (
    <div className="card-base">
      <div className="mb-3 flex items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-slate-800">감사 타임라인</h3>
        <span className="text-xs text-slate-500">최근 {Math.min(maxItems, rows.length)}건 점검</span>
      </div>

      <div className="relative space-y-3 before:absolute before:left-[9px] before:top-1 before:h-[calc(100%-8px)] before:w-px before:bg-slate-200">
        {rows.slice(0, maxItems).map((row) => (
          <div key={row.id} className="relative pl-7">
            ...
            <div className="rounded-xl border border-slate-200 bg-white/70 px-3 py-2">
              ...
              {row.detail && <p className="mt-2 text-xs text-slate-700">{row.detail}</p>}
            </div>
          </div>
        ))}
      </div>
