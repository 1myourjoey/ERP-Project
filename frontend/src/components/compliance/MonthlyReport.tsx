    ...
    <div className="space-y-3">
      <div className="card-base">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div>
            <h3 className="text-sm font-semibold text-[#0f1f3d]">월간 준법감시 리포트</h3>
        ...
        <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
            <p className="text-xs text-[#64748b]">총 점검 수</p>
            <p className="text-lg font-semibold text-[#0f1f3d]">{data.summary.total_checks}</p>
          </div>
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
          ...
          <div className="rounded-xl border border-[#d8e5fb] bg-white/70 p-3">
            <p className="text-xs text-[#64748b]">위반/오류</p>
      ...
      <div className="card-base">
        <h4 className="mb-2 text-sm font-semibold text-[#0f1f3d]">전월 대비 추이</h4>
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="rounded-lg border border-[#d8e5fb] bg-[#f5f9ff] p-2 text-xs text-[#0f1f3d]">
      ...
      <div className="card-base overflow-auto">
        <h4 className="mb-2 text-sm font-semibold text-[#0f1f3d]">위반 내역</h4>
        {!data.violations.length ? (
          <p className="text-xs text-[#64748b]">해당 월에 위반 내역이 없습니다.</p>
        ) : (
              ...
              {data.violations.map((row) => (
                <tr key={row.check_id}>
                  <td className="px-3 py-2">
                    <div className="font-mono text-xs text-[#64748b]">{row.rule_code || '-'}</div>
                    <div>{row.rule_name || '-'}</div>
                  </td>
                  <td className="px-3 py-2">
                    <span className="tag tag-red">{row.result || '-'}</span>
                  </td>
                  <td className="px-3 py-2 text-xs text-[#0f1f3d]">{row.detail || '-'}</td>
