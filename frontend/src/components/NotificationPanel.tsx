...
export default function NotificationPanel({ onClose }: NotificationPanelProps) {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 z-50 w-[22rem] max-h-[480px] card-base shadow-xl flex flex-col">
      ...
      <div className="flex gap-1 px-3 py-2 border-b border-[var(--theme-border)]">
        {TABS.map((tab) => (
          <button
            key={tab.key}
            type="button"
            className={`rounded px-2 py-1 text-xs transition-colors ${
              activeTab === tab.key
                ? 'bg-[var(--theme-hover)] text-[var(--theme-text-primary)]'
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading && (
          <div className="px-4 py-10 text-center text-sm text-[var(--theme-text-secondary)]">
            알림을 불러오는 중...
          </div>
        )}
